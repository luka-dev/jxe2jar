...
- `mount-fs/run.lnk` Shortcut with the current `jar2jxe.exe` arguments (WM5 not allows direct CLI or BAT files. Solution to install TotalCommanderCE tcmdpocketarm.cab, and run through link from it).
...
- For file transfer to/from XP, you can use `copyparty` (works with IE, runes from host machine).
- The WM5 emulator supports folder mapping form XP and exposed as `Storage Card`.
